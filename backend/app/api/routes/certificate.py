...
async def mint_certificate(
    ...
    if not verify_solana_signature(sub["wallet"], body_bytes.decode() or submission_id, x_signature):
    ...
    tx_sig = await issue_certificate_on_chain(submission_id, metadata_uri)
    if not tx_sig:
